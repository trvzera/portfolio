...
function startBackgroundVideo() {
  ...
  backgroundVideo.src = new URL(
    "../media/videos/backloop.webm",
    import.meta.url,
  );
...
window.addEventListener("performance-mode-change", (event) => {
  const loopingContentVideos = document.querySelectorAll(
    "video[loop]:not(.site-background-video)",
  );
...
document.addEventListener("keydown", (event) => {
  if (
    ...
    document.body.classList.contains("is-page-leaving")
  )
    return;
...
const defaultProfilePhoto = new URL(
  document.querySelector("#main")
    ? "../imgs/banners/giovanni.webp"
    : "../imgs/banners/about2.webp",
  import.meta.url,
);
const footerProfilePhoto = new URL(
  "../imgs/banners/about1.webp",
  import.meta.url,
);
const nameNodes = [];
const nameWalker = document.createTreeWalker(
  document.body,
...
while (nameWalker.nextNode()) {
  ...
  if (
    node.textContent.includes("Giovanni Trivellato") &&
    !parent.closest(
      "script, style, .about-name, .profile-name, .about-eyebrow, .projects-eyebrow, .skills-eyebrow, .contact-eyebrow",
    )
  )
    nameNodes.push(node);
}
...
for (const node of nameNodes) {
  ...
  parts.forEach((part, index) => {
    ...
    photo.src = node.parentElement.closest("footer")
      ? footerProfilePhoto
      : defaultProfilePhoto;
    photo.alt = "";
    name.append(photo);
...
if (!reducedMotion.matches && "IntersectionObserver" in window) {
  ...
  revealTargets.forEach((target, index) => {
    if (
      target.classList.contains("motion-card") ||
